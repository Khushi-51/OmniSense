...
export const VoiceInterface: React.FC<VoiceInterfaceProps> = ({
  ...
  const [lastProcessedCommandIndex, setLastProcessedCommandIndex] =
    useState(-1);

  // Handle new voice commands
  useEffect(() => {
    if (commands.length > 0) {
      const latestCommandIndex = commands.length - 1;

      // Only process if this is a new command we haven't processed yet
      if (latestCommandIndex > lastProcessedCommandIndex) {
        const latestCommand = commands[latestCommandIndex];
        const commandText = latestCommand.text.trim();

        // Only process if command has meaningful text (at least 2 characters) and good confidence
        if (commandText.length >= 2 && latestCommand.confidence > 0.7) {
          vibrateSuccess?.();
          onCommand?.(commandText);
          setLastProcessedCommandIndex(latestCommandIndex);
  ...
  useEffect(() => {
    if (
      response &&
      audioEnabled &&
      response !== lastSpokenRef.current &&
      speak &&
      !isSpeaking
    ) {
      speak(response);
      ...
      <div className="relative">
        ...
        >
          ...
          <span className="text-lg flex items-center gap-2">
            {isProcessing && (
              <span className="w-4 h-4 border-2 border-t-2 border-blue-500 rounded-full animate-spin"></span>
            )}
            {isProcessing
              ? "Processing..."
              : isProcessingCommand
