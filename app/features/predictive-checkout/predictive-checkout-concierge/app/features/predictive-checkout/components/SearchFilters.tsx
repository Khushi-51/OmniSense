...
export default function SearchFilters({ filters, onFiltersChange, isOpen, onClose, facets }: SearchFiltersProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 lg:relative lg:inset-auto">
      ...
      {/* Filter Panel */}
      <div className="fixed right-0 top-0 h-full w-80 bg-gray-900 border-l border-gray-700 overflow-y-auto lg:relative lg:w-full lg:h-auto lg:border-l-0 lg:border lg:border-gray-700 lg:rounded-xl">
        {/* Header */}
        <div className="sticky top-0 bg-gray-900 border-b border-gray-700 p-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button onClick={onClose} className="p-1 text-gray-400 hover:text-white transition-colors lg:hidden">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="p-4 space-y-6">
                ...
                {SEARCH_FILTERS.brands.map((brand) => (
                  <label key={brand} className="flex items-center space-x-2 cursor-pointer">
                    <input
                      ...
                      className="rounded border-gray-600 bg-gray-800 text-purple-500 focus:ring-purple-500"
                    />
                    <span className="text-sm text-gray-300 flex-1">{brand}</span>
                ...
                {SEARCH_FILTERS.dietary.map((diet) => (
                  <label key={diet} className="flex items-center space-x-2 cursor-pointer">
                    <input
                      ...
                      className="rounded border-gray-600 bg-gray-800 text-purple-500 focus:ring-purple-500"
                    />
                    <span className="text-sm text-gray-300 capitalize flex-1">{diet.replace("-", " ")}</span>
          ...
          {/* Additional Filters */}
          <div>
            <h4 className="font-medium text-white mb-3">Additional Filters</h4>
            <div className="space-y-2">
