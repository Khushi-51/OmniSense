...
export default function BudgetMonitor({ budget }: BudgetMonitorProps) {
  ...
  return (
    <div className="bg-gray-900 rounded-xl shadow-lg p-4 border border-gray-800 mx-2 my-3">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h2 className="text-lg font-semibold text-white">Budget Monitor</h2>
            <p className="text-xs text-gray-400">Track your spending in real-time</p>
          </div>
        </div>
      ...
      <div className="flex flex-col items-center mb-4">
        ...
        <div className="grid grid-cols-2 gap-3 w-full text-center mt-3">
          <div className="p-3 bg-gray-800 rounded-lg">
            ...
            <p className="text-lg font-bold text-white break-words">₹{budget.spent.toFixed(2)}</p>
          </div>
          <div className="p-3 bg-gray-800 rounded-lg">
            <p className="text-xs text-gray-400 mb-1">Remaining</p>
            <p className="text-lg font-bold text-white break-words">₹{budget.remaining.toFixed(2)}</p>
          </div>
        </div>

        <div className="w-full mt-3 space-y-2">
          <div className="flex justify-between items-center text-xs">
            <span className="text-gray-400 flex-shrink-0">Budget Limit:</span>
            <span className="font-semibold text-white text-right">₹{budget.limit.toFixed(2)}</span>
          </div>

          {budget.savings > 0 && (
            <div className="flex justify-between items-center text-xs">
              <span className="text-gray-400 flex-shrink-0">Total Savings:</span>
              <span className="font-semibold text-green-400 text-right">₹{budget.savings.toFixed(2)}</span>
            </div>
          )}

          <div className="flex justify-between items-center text-xs">
            <span className="text-gray-400 flex-shrink-0">Projected Total (with tax):</span>
            <span className="font-semibold text-white text-right">₹{budget.projectedTotal.toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      {showTips && tips.length > 0 && (
        <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-3">
          <h4 className="font-medium text-blue-300 mb-2 flex items-center text-sm">
            <Lightbulb className="w-3 h-3 mr-1" />
            Smart Shopping Tips
          </h4>
          <ul className="space-y-1">
            {tips.map((tip, index) => (
              <li key={index} className="text-xs text-blue-200 flex items-start">
